...
tab:RegisterGrid(15, {
	OnUpdate = function() 
			tab:SetStatus(format("%s%s", colors.white, L["Profession Equipment"]))
		end,
	GetSize = function() return ProfessionEquipment.GetNumSlotTypes() end,
	RowSetup = function(self, rowFrame, dataRowID)
			local name, color = ProfessionEquipment.GetSlotTypeInfo(dataRowID)
	...
	ColumnSetup = function(self, button, dataRowID, character)
			...
			if item then
				...
				button.key = nil
				button.Background:SetTexture(ProfessionEquipment.GetSlotIcon(dataRowID))
				button.Name:SetText("")
			end
			
			button.id = inventoryId --dataRowID
		end,
